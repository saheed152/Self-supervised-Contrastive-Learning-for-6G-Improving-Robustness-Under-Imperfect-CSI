...
% paper; Modify it to export the array index for each realizations; modify
% for single snr to generate the test set; set snr=20dB
...
load("ICC_channel_Nr16_1000real.mat"); %load the 
...
for z=1:realization_brute
    ...
    for j = 1:size(ind,1)
        ...
        s=8; %selecting the snr=20db
            ...
            if (sum(abs(R_sumo)) > sum(abs(Max_R)))
                Max_R = abs(R_sumo);
